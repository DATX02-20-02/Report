...
\input{tex/introduction/theory.tex}
...
% With the help of PCG, the software can produce impressive, varied and seemingly endless environments order of magnitudes faster than a human~\cite[p.3]{PCG_in_games}.
% This aids designers to create a lot more content cheaply and efficiently once the algorithm has been developed.
% Additionally, as soon as the algorithm is done it can be reused. Similar to a debugger, a PCG algorithm can be infinitely run afterward. The algorithm can potentially be reused to generate completely different content.

% Although the output space of PCG is considerable, an automatic content generation comes with a set of obstacles.
