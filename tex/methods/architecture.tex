...
However, this lack of overhead could impose limitations and there still needed to exist some modules responsible for receiving and exporting all the generated model data.
...
Function-based architecture showed several promising advantages.
For one, each generator would only receive input that it depended on, while with a pipeline each generator would have had to pass along all its data.
...
Consequently, function-based architecture was chosen.
